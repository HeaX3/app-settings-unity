...
namespace AppSettings
{
    ...
    {
        private readonly Dictionary<SettingType, SettingValue> _values = new();

        public string File { get; }

        public SettingsContainer(string file, params SettingType[] entries) : this(file,
            (IEnumerable<SettingType>)entries)
        {
        }
        ...
        {
            return _values.TryGetValue(setting, out var v) && v is SettingValue<T2> result
                ? result.Value
        ...
        public void Load(JObject data)
        {
            foreach (var entry in _values)
            {
                ...
                entry.Value.LoadValue(data[key.Key]);
            }
        }
        ...
        public void Load(JArray data)
        {
            ...
            {
                if (!(entry is JObject section)) continue;
                var key = section["key"] != null ? (string)section["key"] : null;
                var value = section["value"];
                if (key == null) continue;
                var setting = _values.FirstOrDefault(v => v.Key.Key == key);
                if (setting.Value == null) continue;
                setting.Value.LoadValue(value);
            }
        }
